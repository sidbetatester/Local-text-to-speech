# 🔊 Private Voice Reader by Sid

Private Voice Reader is a privacy-focused browser app for local text-to-speech, reader mode, and optional voice input.
...
### 🔊 Private Voice Reading (100% Local)
...
This project is licensed under the **Private Voice Reader Source-Available License v1.0**.
...
# 🔊 Private Voice Reader

Privacy-first, single-file browser app for local text-to-speech, reader mode, and optional speech-to-text.
...
## Backup / Export
- Export Backup creates a versioned JSON bundle with `format: private-voice-reader-backup`
...
- Current frozen hashes for `index.html`:

```text
style-src  'sha256-6NR6mQ4GyJsW6HG20UBeKQjQSH2024RAIjJDXEPVLBU='
script-src 'sha256-/hx6zJYYeCi/3TL4znAxcCoNRdrudBla1pZwG1Fe3xw=' 'sha256-YlwXUiaiQqprdpERYLNSZjssxHYzSUSLfG4L9XPtlZ4='
```

Example header:

```text
Content-Security-Policy: default-src 'self'; script-src 'self' 'sha256-/hx6zJYYeCi/3TL4znAxcCoNRdrudBla1pZwG1Fe3xw=' 'sha256-YlwXUiaiQqprdpERYLNSZjssxHYzSUSLfG4L9XPtlZ4='; style-src 'self' 'sha256-6NR6mQ4GyJsW6HG20UBeKQjQSH2024RAIjJDXEPVLBU='; worker-src 'self' blob:; img-src 'self' data:; connect-src 'self';
```
...
Made with ❤️ by [Sid](https://github.com/sidbetatester) for privacy-first voice reading
